defmodule Hailstorm.PromEx do
  ...
  1. Update your configuration (config.exs, dev.exs, prod.exs, releases.exs, etc) to
     ...
     ```
     config :hailstorm, Hailstorm.PromEx,
       disabled: false,
  ...
  3. Update your `endpoint.ex` file to expose your metrics (or configure a standalone
     ...
     ```
     defmodule HailstormWeb.Endpoint do
       use Phoenix.Endpoint, otp_app: :hailstorm
  ...
  def dashboards do
    [
      ...
      # {:hailstorm, "/grafana_dashboards/user_metrics.json"}
    ]
  end
end
